...
if __name__ == '__main__':
    ...
    model2 = model = SDFNetwork(tcnn_network, encoding="hashgrid")
    models = [model, model2]
    print(model)
    ...
    if opt.test:
        ...
        valid_loaders = []
        for i in range(n):
            train_dataset = SDFDataset(paths[i], size=10, num_samples=2**18)
        ...
        trainer = Trainer('ngp', n,models, workspace=opt.workspace1, enc_optimizer=enc_optimizer,  net_optimizer=net_optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint='latest', eval_interval=1)

        trainer.train(train_loaders, valid_loaders, 5)
