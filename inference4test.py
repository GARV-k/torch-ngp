...
def save_mesh(models, mode,no_of_rots,W=None,N=None,M=None,save_path='inferences/', resolution=128):
...
if __name__ == '__main__':
    ...
    workspace1 = f'test/'
    workspace2 = f'hash_workspace_obj12_2_3_10/'
    ...
    # w = torch.load('w.pth')
    W= torch.load(workspace2+'W.pth')
    # W = torch.matmul(W,W)# W2 = 2*W
    
    #trainer.save_mesh(os.path.join(workspace1, 'results', 'output.ply'), 1024)
    save_mesh(models,mode,no_of_rots,W,N,M,save_path='inferences/test/')     
