...
class SDFNetwork(nn.Module):
    ...
    def forward(self, x,mode='normal',W=None,N=None,M=None,B1=None,B2=None,ret_new=False):   
        # x: [B, 3]

        x = (x + 1) / 2 # to [0, 1]
        # if N != None:
        if ret_new:
            new_hash,x = self.encoder(x,mode,W,N,M,B1,B2,ret_new)
        else:
            x = self.encoder(x,mode,W,N,M,B1,B2,ret_new)
        # else:
        #     x = self.encoder(x)
        h = self.backbone(x)
