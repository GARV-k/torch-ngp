...
class Trainer(object):
    ...
    def w_train_step(self,mode, data, model,W=None,N=None,M=None,B1=None,B2=None,gt_hash=None):
        # assert batch_size == 1
        ...
        lam = 1
        #pred = self.model(X)
        new_hash,pred = model(X,mode,W,N,M,B1,B2,True)
        # l2 = torch.nn.functional.mse_loss(new_hash, gt_hash)
        # loss = self.criterion(pred, y) + (lam)*l2 
        loss = self.criterion(pred, y)
        return pred, y, loss
    ...
    def w_forward_epoch_gradaccum(self,mode,subset_loaders_iter, subset_models, div_factor):
        ...
        for j, loader_iter in enumerate(subset_loaders_iter):
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    # if j%4!=0:
                    #     preds, truths, loss = self.w_train_step(mode,data,subset_models[j],self.W,self.N,self.M,self.B1,self.B2,subset_models[j-1].encoder.hash_table)    
                    # else:
                    #     preds, truths, loss = self.w_train_step(mode,data,subset_models[j],self.W,self.N,self.M,self.B1,self.B2,subset_models[j+3].encoder.hash_table)
    ...
    def w_train_one_epoch(self, mode, multiple_rots,loaders, models, macro_batch_size):
        ...
        for i in range(N):
            ...
            #         if hasattr(self, "W") and self.W.requires_grad and self.W.grad is not None:
            #             # Log gradient distribution
            #             self.writer.add_histogram("train/W_grad", self.W.grad, self.global_step)
            #             # Optionally log gradient norms
            #             grad_norm = self.W.grad.norm().item()
            #             self.writer.add_scalar("train/W_grad_norm", grad_norm, self.global_step)
